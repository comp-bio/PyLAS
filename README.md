...
`pylas.py` — Подсчёт итоговой локальной метрики LAS средствами python3  
...
python3 pylas.py \
  [файл в формате .bcov] \
  [степень 2, которая будет использована как размер региона (N для 2^N)] \
  [среднее покрытие по геному] \
  [название хромосомы]
```
...
```bash
python3 pylas.py data/mini.bcov 8 30 mini > result.tsv
```
